...
export default function Lab(){
 ...
 <main><div className="intro compact-intro"><div className="intro-actions"><button type="button" className="export-results" disabled={busy} onClick={()=>exportData()}><Download size={15}/> Export results</button><button type="button" className="primary upload-video" disabled={busy} onClick={()=>upload.current?.click()}><Upload size={20}/> Upload video</button><input ref={upload} type="file" accept="video/mp4,video/webm,.mp4,.webm" hidden onChange={event=>{const file=event.target.files?.[0];if(file)analyze(file);}}/></div></div>
