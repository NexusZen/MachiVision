...
VERSION='visual-reference-v1-signed'
...
def reference_features(name,step):
    ...
    if name=='Motion pulse' and on:f.update(motion_magnitude=1,contrast=1,temporal_change=.3)
    if name=='Looming pulse' and on:f.update(looming=1,contrast=1,temporal_change=.3)
    if name=='Brightness transitions' and on and step%10<2:f.update(temporal_change=1,flicker=1)
    if name=='Combined visual drive' and on:f.update(motion_magnitude=1,looming=1,contrast=1,temporal_change=1)
...
def calibration_for(graph,parameters):
    p=dict(decay=.65,gain=.65,input=1,threshold=5,hops=6)
    ...
    for name in NAMES:
        ...
        for step in range(90):
            sim.step(reference_features(name,step))
            peak=max(peak,float(sim.a[ids].mean()*100) if ids else 0)
