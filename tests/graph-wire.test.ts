...
import {Graph,defaults,simulate,Features} from '../lib/model';
...
test('compact transport preserves real neuron IDs, every edge, and simulated activity',()=>{
 ...
 const previous=graph.neurons.map((_,i)=>(i%11)/11);
 assert.deepEqual(simulate(f,previous,restored,defaults),simulate(f,previous,graph,defaults));
